...
let g:active_keys = {}
" dict containing mapping as expected in the 'map' command output
let g:mapping_dict = {}

" state (prev_motion is literally the vim valid motion as per ';' and ','
let g:prev_motion = ''
"let g:last_keyseq = ''
...
function! s:getkeys()
  "display all the active mappings
  " :Breakadd here
  echo "Forward and reverse repeatable motions:"
...
" "[[" and "]]" are already mapped for vim filetypes and would error.
" Could the he mapping command be fixed
" see :h maxmapdepth
if &filetype == "vim"
...
for k in keys(b:default_mappings)
  ...
  " Special handling for f,F and t,T because I swapped them in my layout
  " and it would recursely get remapped otherwise. NOTE: could it be avoided?
  ...
  nnoremap f :call <SID>sendkeys('f')<CR>
  nnoremap F :call <SID>sendkeys('F')<CR>
  nnoremap t :call <SID>sendkeys('t')<CR>
  nnoremap T :call <SID>sendkeys('T')<CR>

  " other special mappings
  nnoremap <PageDown> <cmd>call <SID>sendkeys("\<C-D>")<CR>
  nnoremap <PageUp> <cmd>call <SID>sendkeys("\<lt>C-U>")<CR>
...
command ListRepeatableMotionsMappings call <SID>getmappings()

" Default Mapping
nnoremap  <silent> <Space> <cmd>call <SID>repeat()<CR>
nnoremap <silent> <BS> <cmd>call <SID>reverserepeat()<CR>

" NOTE: <S-Space> and <S-BS> not detected by the gnome terminal The sequences
" generated by key presses are hardcoded within the VTE source code (src/keymap.cc).
nnoremap <silent> <S-Space> <cmd>call <SID>reverserepeat()<CR>
